...
type IOCountersStat struct {
	Name        string `json:"name" bson:"name"`               // interface name
	BytesSent   uint64 `json:"bytesSent" bson:"bytesSent"`     // number of bytes sent
	BytesRecv   uint64 `json:"bytesRecv" bson:"bytesRecv"`     // number of bytes received
	PacketsSent uint64 `json:"packetsSent" bson:"packetsSent"` // number of packets sent
	PacketsRecv uint64 `json:"packetsRecv" bson:"packetsRecv"` // number of packets received
	Errin       uint64 `json:"errin" bson:"errin"`             // total number of errors while receiving
	Errout      uint64 `json:"errout" bson:"errout"`           // total number of errors while sending
	Dropin      uint64 `json:"dropin" bson:"dropin"`           // total number of incoming packets which were dropped
	Dropout     uint64 `json:"dropout" bson:"dropout"`         // total number of outgoing packets which were dropped (always 0 on OSX and BSD)
	Fifoin      uint64 `json:"fifoin" bson:"fifoin"`           // total number of FIFO buffers errors while receiving
	Fifoout     uint64 `json:"fifoout" bson:"fifoout"`         // total number of FIFO buffers errors while sending
...
type ConnectionStat struct {
	Fd     uint32  `json:"fd" bson:"fd"`
	Family uint32  `json:"family" bson:"family"`
	Type   uint32  `json:"type" bson:"type"`
	Laddr  Addr    `json:"localaddr" bson:"localaddr"`
	Raddr  Addr    `json:"remoteaddr" bson:"remoteaddr"`
	Status string  `json:"status" bson:"status"`
	Uids   []int32 `json:"uids" bson:"uids"`
