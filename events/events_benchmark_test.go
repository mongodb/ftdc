			...
			for _, test := range []struct {
				...
				},
				{
					...
					Generator: func() interface{} {
						...
						for i := int64(1); i <= 10; i++ {
							point.Add(fmt.Sprintln("stat", i), rand.Int63n(i))            // no lint
							point.Add(fmt.Sprintln("stat", i), float64(i)+rand.Float64()) // no lint
						}
						return point
					},
				},
				...
				b.Run(test.Name, func(b *testing.B) {
					b.Run("Add", func(b *testing.B) {
						for n := 0; n < b.N; n++ {
							collector.Add(test.Generator()) // no lint
						}
					})
					b.Run("Resolve", func(b *testing.B) {
						for n := 0; n < b.N; n++ {
							collector.Resolve() // no lint
						}
					})
				})
			}
